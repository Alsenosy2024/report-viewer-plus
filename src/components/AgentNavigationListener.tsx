...
export const AgentNavigationListener = () => {
    ...
    try {
      ...
      const executeNavigation = () => {
        ...
        const attemptNavigation = () => {
          ...
          // CRITICAL: Call navigate() immediately - no delay
          // The blocking mechanisms should prevent old navigations from executing
          try {
            ...
            console.log(`[Agent Navigation] 📍 Calling navigate(${pathname}, { replace: true }) IMMEDIATELY...`);
  ...
  const processTranscriptMessages = useCallback(() => {
    ...
    newMessages.forEach((message, relativeIndex) => {
      ...
      if (message.role === 'assistant') {
        ...
        // CRITICAL: Skip ALL messages that contain navigation commands
        // Navigation is handled ONLY via data channel, not transcript
        const navMatch = content.match(/NAVIGATE:(\/[^\s]*)/);
        if (navMatch) {
          console.log('[Agent Navigation] ⚠️⚠️⚠️ SKIPPING TRANSCRIPT MESSAGE WITH NAVIGATION ⚠️⚠️⚠️');
          console.log('[Agent Navigation]   Navigation handled via data channel only');
          console.log('[Agent Navigation]   Message path:', navMatch[1]);
          console.log('[Agent Navigation]   Marking as processed and skipping');
  ...
  // DISABLED FOR NAVIGATION - Navigation is handled ONLY via data channel
  useEffect(() => {
    ...
    // DISABLED: Transcript polling for navigation causes flash
    // Navigation is handled ONLY via data channel (RoomEvent.DataReceived and useDataChannel)
    // Only process transcript for non-navigation messages
    console.log('[Agent Navigation] ⚠️ Transcript polling DISABLED for navigation - using data channel only');
  ...
  useEffect(() => {
    ...
    // DISABLED: DOM mutation observer causes flash
    // Navigation is handled ONLY via data channel (RoomEvent.DataReceived and useDataChannel)
    console.log('[Agent Navigation] ⚠️ DOM mutation observer DISABLED for navigation - using data channel only');
