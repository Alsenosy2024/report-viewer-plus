...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'emails') THEN
    ...
    EXECUTE 'CREATE POLICY "Only admins can access emails" ON public.emails FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE profiles.id = auth.uid() AND profiles.role = ''admin''))';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'email2') THEN
    ...
    EXECUTE 'CREATE POLICY "Only admins can access email2" ON public.email2 FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE profiles.id = auth.uid() AND profiles.role = ''admin''))';
  END IF;
END $$;
