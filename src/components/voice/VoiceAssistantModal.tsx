        ...
        try {
          ...
          if (audioTrack) {
            ...
            if (isMutedRef.current && actualTrackEnabled) {
              ...
              // Both agree on muted - keep it muted
              console.log('[VoiceAssistant] ⚠️ Mic is muted - keeping track disabled');
            }
...
export const VoiceAssistantModal: React.FC<VoiceAssistantModalProps> = ({ open, onOpenChange }) => {
  ...
  useEffect(() => {
    if (open && !token && !tokenLoading) {
      console.log('[VoiceAssistant] Modal opened, initiating connection...');
      handleConnect();
    }
  }, [open, token, tokenLoading, handleConnect]);
